...
public class EventManager {
    ...
    public String validateEvent(String title, String date, String time, String venue, String capacity, String price) {
        ...
        try { LocalDate.parse(date.trim(), EVENT_DATE_FORMAT); } catch (DateTimeParseException e) { return "Please enter a valid event date in the format dd/MM/yyyy."; }
        try { LocalTime.parse(time.trim(), EVENT_TIME_FORMAT); } catch (DateTimeParseException e) { return "Please enter a valid event time in the format HH:mm."; }
        ...
        } catch (NumberFormatException e) { return "Capacity must be a whole number."; }
        try {
            if (Double.parseDouble(price.trim().replace(",", ".")) < 0) return "Price cannot be negative.";
        } catch (NumberFormatException e) { return "Price must be a number."; }
        return null;
    }
    ...
    public boolean isValidEmail(String email) {
        ...
        return !trimmed.isEmpty() && trimmed.contains("@") && trimmed.contains(".") && trimmed.indexOf('@') > 0 && trimmed.indexOf('.') > trimmed.indexOf('@') + 1;
    }
    ...
    public TicketPurchase createTicketPurchase(Event event, String customerName, String customerEmail, String ticketType, int quantity) {
        return new TicketPurchase(event, customerName.trim(), customerEmail.trim(), ticketType.toUpperCase(Locale.ENGLISH), quantity, calculateTotalPrice(event, ticketType, quantity));
    }
